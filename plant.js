function Plant(data) {
	...
	this.drawPlant = function(canvas) {
		...
		for(i=200000;i<data.length;i++) {
			...
			context.beginPath();
			if (data[i][0]%60 < 50) {
      		context.rect(Math.floor(data[i][0]/60) * 6 + 12, data[i][0]%60 * 6 + 150, 5, 5);
      			context.fill();
      			context.lineWidth = 1;
      		}
...
redrawAPlant = function(which, value, day) {
	...
	context.beginPath();
	if (value == 0) {
		context.fillStyle = "rgba(255,0,0," + plants[which][3]/50 + ")";
	} else if (value == 1) {
		context.fillStyle = "rgba(0,255,0," + plants[which][3]/50 + ")";
	} else if (value == 2) {
		context.fillStyle = "rgba(0,0,255," + plants[which][3]/50 + ")";
	} else if (value == 3) {
		context.fillStyle = "rgba(255,255,0," + plants[which][3]/50 + ")";
	} else if (value == 4) {
		context.fillStyle = "rgba(120,20,120," + plants[which][3]/50 + ")";
	}
	if (plants[which][3]<1.5) {
		context.fillStyle = "rgb(255,255,255)";
	}
	context.beginPath();
    context.rect(Math.floor(which/60) * 6 + 12, which%60 * 6 + 150, 5, 5);
	context.fill();
    context.lineWidth = 3;
    context.strokeStyle = 'black';
    //context.stroke();
