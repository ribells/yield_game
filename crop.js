	...
	this.drawCrop = function(canvas) {
		...
		for(i=20000;i<data.length;i++) {
			...
			if (seed[(data[i][3])][0] == 0) {
				...
				context.fillStyle = "rgba(120,20,120,0.5)";
			}
			context.beginPath();
      		context.rect(data[i][1] * 8 + 460, data[i][2] * 6 + 60, 8, 6);
      		context.fill();
      		context.lineWidth = 1;
...
redrawACrop = function(which, value, day) {
	...
	context.beginPath();
	if (value == 0) {
		context.fillStyle = "rgba(255,0,0," + crops[which][5]/50 + ")";
	} else if (value == 1) {
		context.fillStyle = "rgba(0,255,0," + crops[which][5]/50 + ")";
	} else if (value == 2) {
		context.fillStyle = "rgba(0,0,255," + crops[which][5]/50 + ")";
	} else if (value == 3) {
		context.fillStyle = "rgba(255,255,0," + crops[which][5]/50 + ")";
	} else if (value == 4) {
		context.fillStyle = "rgba(120,20,120," + crops[which][5]/50 + ")";
	}
    ...
    context.rect(crops[which][1] + 410, crops[which][2] + 27, 5, 5);
	context.fill();
    context.lineWidth = 1;
    context.strokeStyle = 'black';
    //context.stroke();
...
plantCrops = function(lat, lon, x_seq, y_seq, value) {
	...
	if(which==-1) {
		...
		crops.push([crops.length, lon, lat, value, crops.length, 0, 0, 0]);
		x_seq=1;
		y_seq=1;
